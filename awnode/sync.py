...
class WorkspaceConfig:
    ...
    # {base_url, model, api_key} for an operator-configured OpenAI-compatible backend.
    # HOLDS A SECRET — see `_save_local_state`, which is why that method now sets 0600.
    custom_backend: Dict[str, Any] = field(default_factory=dict)
...
class WorkspaceSyncClient:
    ...
    def _load_local_state(self):
        ...
        if SYNC_STATE_FILE.exists():
            try:
                ...
                cb = data.get("custom_backend")
                self.workspace.custom_backend = cb if isinstance(cb, dict) else {}
            except Exception:
                pass
    ...
    def _save_local_state(self):
        ...
        # The operator's BYO-backend API key lives here. Only persisted when a base_url
        # is actually set — a key with no host to go to is a secret held for no reason.
        if self.workspace.custom_backend.get("base_url"):
            data["custom_backend"] = self.workspace.custom_backend
        # 0600 BEFORE the write, not after.
        #
        # THIS FILE ALREADY HELD A BEARER TOKEN AND WAS WORLD-READABLE. Measured
        # 2026-09-13 while adding `custom_backend`: `_save_local_state` has always
        # written `tenant_scoped_token` — the credential `server.py`'s remote /chat auth
        # reads back — through a plain `open(..., "w")` with no mode restriction, so on
        # any multi-user box every account could read the node's enrollment token. The
        # BYO key is the second secret in here, not the first; chmod'ing on the way past
        # fixes both. Done before the content is written (O_CREAT with mode 0o600) so
        # there is no window in which the bytes exist at the default umask.
        fd = os.open(SYNC_STATE_FILE, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
        with os.fdopen(fd, "w") as f:
            json.dump(data, f, indent=2)
        try:
            os.chmod(SYNC_STATE_FILE, 0o600)   # tighten a pre-existing looser file
        except OSError as exc:
            # Windows/exotic filesystems ignore POSIX modes; the write above still
            # succeeded, and failing the whole save over a mode bit would be worse.
            # Said out loud rather than swallowed: this file holds two credentials, so
            # "we could not restrict it" is exactly the kind of thing a silent handler
            # turns into "the feature is off".
            logger.debug("could not chmod %s to 0600: %s", SYNC_STATE_FILE, exc)
