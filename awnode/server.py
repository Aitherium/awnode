...
# Any OpenAI-compatible server the OPERATOR configured: their OpenAI account, a
# Together/Fireworks endpoint, a vLLM on another box. Until now awnode's backend
# ladder was a closed enum of six things we ship, so a node owner who already pays
# for inference had no way to make this node use it — and the browser's `local`
# rung, which is just this node, inherited that limitation.
#
# Seeded from the environment and overridable at runtime by `PUT /config`
# (`custom_backend`), which is how the browser's backend picker pushes the same
# choice down here. The API KEY IS NEVER LOGGED: `_custom_summary()` is the only
# thing allowed near a log line or an HTTP response, and it emits a shape, not a value.
CUSTOM_BACKEND_URL = os.environ.get("AITHERNODE_CUSTOM_BACKEND_URL", "")
CUSTOM_BACKEND_MODEL = os.environ.get("AITHERNODE_CUSTOM_BACKEND_MODEL", "")
CUSTOM_BACKEND_API_KEY = os.environ.get("AITHERNODE_CUSTOM_BACKEND_API_KEY", "")
...
# The live custom-backend config. A module-level dict rather than a BackendState
# attribute because it holds a SECRET: keeping it in one named place makes "what can
# reach the key" a question with a grep-able answer.
_CUSTOM: Dict[str, str] = {
    "base_url": CUSTOM_BACKEND_URL.rstrip("/"),
    "model": CUSTOM_BACKEND_MODEL,
    "api_key": CUSTOM_BACKEND_API_KEY,
}


def _custom_summary() -> Dict[str, Any]:
    """The ONLY representation of the custom backend allowed into a log or a response.

    Never the key. `has_api_key` is the whole truth a caller needs — "is one set" is
    actionable, the value is not, and a key in a log is a key in a backup, a support
    bundle and a screenshot. Same rule the platform applies to every other secret.
    """
    return {
        "base_url": _CUSTOM["base_url"],
        "model": _CUSTOM["model"],
        "has_api_key": bool(_CUSTOM["api_key"]),
    }


def _is_safe_custom_base(url: str) -> bool:
    """https anywhere, plain http only to this machine.

    The mirror of `isAllowedCustomOrigin` in AitherVeil's `resolve-node-endpoint.ts`,
    and asserted by the same reasoning: the credential travelling to this URL belongs to
    the node's owner, so the question is not "is this host ours" but "can this key be
    read off the wire". A LAN IP is the trap — it feels local and is not encrypted.
    """
    try:
        parsed = urlparse(url)
    except ValueError:
    ...
    if host in ("localhost", "::1"):
        return True
    return bool(re.match(r"^127\.\d{1,3}\.\d{1,3}\.\d{1,3}$", host))


def _custom_auth_headers(base: str) -> Dict[str, str]:
    """The user's key, and ONLY for the user's own host.

    Keyed on an exact base match rather than "is a custom backend configured", so a
    turn that routed to vLLM or Ollama can never pick up the key on its way out. That
    is the same failure shape as sending a platform bearer to a non-allowlisted origin,
    one direction over.
    """
    if not base or not _CUSTOM["base_url"]:
        return {}
    if base.rstrip("/") != _CUSTOM["base_url"].rstrip("/"):
        return {}
    key = _CUSTOM["api_key"]
...
class BackendState:
    ...
    async def refresh(self):
        # The custom backend is probed by the SAME rule as every other OpenAI-compatible
        # one (`/v1/models == 200`, never `/health`) — a configured URL that does not
        # answer must not be reported as a backend, or the node advertises a rung that
        # 503s every turn.
        self.custom = bool(_CUSTOM["base_url"]) and await _probe_openai(_CUSTOM["base_url"])
        ...
        # A backend the OPERATOR configured outranks everything we auto-detect, in
        # every mode that is not explicitly "cloud". Anything else is a silent
        # fallback: they named a backend, and quietly preferring a co-resident Ollama
        # because it happens to be running is us overriding an explicit choice.
        if MODE != "cloud" and self.custom:
            self.mode = "custom"
        elif MODE == "cloud":
...
def _seed_custom_from_disk() -> None:
    """Restore a `PUT /config` custom backend across a restart.

    ENV WINS WHEN SET, because that is the operator configuring the PROCESS and it must
    not be silently overridden by something a browser pushed months ago. With no env,
    the persisted value is restored — otherwise "save" in the picker would survive
    exactly until the next node restart, which is the shape of a setting that appears
    ...
    try:
        ...
        sync._load_local_state()
        stored = sync.workspace.custom_backend or {}
    except Exception as exc:  # noqa: BLE001 — a bad state file must not stop the node
        logger.warning("could not restore custom backend from sync state: %s", exc)
        return
    base = str(stored.get("base_url") or "").rstrip("/")
    if not base or not _is_safe_custom_base(base):
        return
    _CUSTOM["base_url"] = base
    _CUSTOM["model"] = str(stored.get("model") or "")
    _CUSTOM["api_key"] = str(stored.get("api_key") or "")


@asynccontextmanager
async def lifespan(app: FastAPI):
    _seed_custom_from_disk()
    ...
    if _CUSTOM["base_url"]:
        # Shape, never the key -- `_custom_summary()` is the only thing allowed here.
        logger.info(
            f"  Custom:   {'UP' if _state.custom else 'DOWN'} ({_custom_summary()})"
        )
...
# Lock CORS to EXACT first-party origins over https + loopback. A wildcard ("*")
# lets ANY website the user visits call this loopback node cross-origin — and since
# every browser request arrives from 127.0.0.1, the loopback owner gate cannot tell
# aitherium.com from a drive-by site. A `*.aitherium.com` regex was not enough either:
# tenant apps are hosted under that wildcard and plain http was admitted, so any such
# page could preflight PUT /config. Only the pinned first-party hosts below (plus
# AITHERNODE_ALLOWED_ORIGINS, comma-separated exact origins) pass; everything else
# fails the preflight AND the Origin check in the owner gate.
FIRST_PARTY_ORIGINS = frozenset({
    ...
    "https://elysium.aitherium.com",
})
# Loopback dev servers (any port). Mirrors _check_origin().
...
def _allowed_origins() -> frozenset:
    extra = {
        ...
        for o in os.environ.get("AITHERNODE_ALLOWED_ORIGINS", "").split(",")
        if o.strip()
    }
    return FIRST_PARTY_ORIGINS | frozenset(extra)


def _origin_allowed(origin: str) -> bool:
    """Exact first-party https origin, or a loopback origin. Fail-closed on anything else."""
...
async def add_private_network_access(request, call_next):
    ...
    if request.url.path == "/health" and origin and _origin_allowed(origin):
...
def _require_node_owner(
    ...
    """Depends() gate for owner-only surfaces: loopback, or the enrolled bearer.

    The container CMD binds 0.0.0.0, so any published port hands these routes to
    non-loopback callers. PUT /config writes the custom backend (base_url + api_key)
    that is rung 0 for every /v1 call; /v1/chat/completions spends the owner's
    backend; /sync/*, /connect, /deploy and /license/validate act on the owner's
    cloud identity. Same contract as /chat and /packs/install. Fail-closed.
    """
...
async def chat(
    ...
    # ── Blank prompt: refuse, do not run the pipeline on nothing ──
    # AFTER the auth gate, so an anonymous remote caller still gets 401 rather than a
    # 400 that tells them the endpoint exists. Measured on the platform's own agentic
    # path 2026-08-30: an empty message ran 7 turns and 7 tools for 133.8s and answered
    # with a first-contact greeting. There is nothing to answer; say so in 1ms.
    if not (req.message or "").strip():
        raise HTTPException(
            status_code=400,
            detail={"error": "Empty message", "empty_message": True},
        )
...
@app.post("/deploy", dependencies=[Depends(_require_node_owner)])
...
@app.post("/connect", dependencies=[Depends(_require_node_owner)])
...
class CustomBackendUpdate(BaseModel):
    """An OpenAI-compatible backend the node OWNER configured.

    `api_key` is write-only by construction: nothing echoes it back (`GET /config` and
    `/status` emit `_custom_summary()`, which carries `has_api_key` and no value), and
    `None` means "leave the stored key alone" while `""` means "forget it". Without
    that distinction, saving a new model would silently wipe the key — the shape where
    a settings form quietly logs you out of your own backend.
    """
    base_url: str = ""
    model: Optional[str] = None
...
class WorkspaceConfigUpdate(BaseModel):
    ...
    custom_backend: Optional[CustomBackendUpdate] = None
...
@app.post("/sync/register", dependencies=[Depends(_require_node_owner)])
...
@app.post("/sync/now", dependencies=[Depends(_require_node_owner)])
async def sync_now():
...
async def get_config():
    ...
    return {
        "workspace": {
            ...
            # Shape only. A settings page needs to know a key IS set, never what it is.
            "custom_backend": _custom_summary(),
...
@app.put("/config", dependencies=[Depends(_require_node_owner)])
async def update_config(update: WorkspaceConfigUpdate):
    ...
    if update.custom_backend is not None:
        cb = update.custom_backend
        base = (cb.base_url or "").strip().rstrip("/")
        if base and not _is_safe_custom_base(base):
            # Refused BEFORE the key is stored, not before it is sent. A key written to
            # disk for an http:// LAN address is already a key that will cross the
            # network in the clear on the first turn.
            raise HTTPException(
                400,
                "custom_backend.base_url must be https://, or http:// only on this "
                "machine (127.0.0.0/8, localhost, ::1) — your API key would otherwise "
                "cross the network unencrypted",
            )
        _CUSTOM["base_url"] = base
        if cb.model is not None:
            _CUSTOM["model"] = cb.model.strip()
        if cb.api_key is not None:
            _CUSTOM["api_key"] = cb.api_key
        if not base:
            # Clearing the URL clears the credential with it: a key with no host to go
            # to is a secret we are holding for no reason.
            _CUSTOM["model"] = ""
            _CUSTOM["api_key"] = ""
        sync.workspace.custom_backend = dict(_CUSTOM)
        # Shape only — NEVER the key.
        logger.info("custom backend updated: %s", _custom_summary())
        await _state.refresh()

    sync._save_local_state()
    # `sync.workspace.__dict__` would carry `custom_backend` INCLUDING the key straight
    # back to the caller (and into whatever logs that response). Redact it here rather
    # than trusting every future caller to remember.
    config = dict(sync.workspace.__dict__)
    if "custom_backend" in config:
        config["custom_backend"] = _custom_summary()
    return {"status": "updated", "config": config, "backend_mode": _state.mode}
...
async def mcp_tools(request: Request):
    ...
    Browser drive-by defense: a request carrying a foreign Origin is another
    site calling this loopback node cross-origin -- refuse it. Origin-less
    callers (CLI, server-side fetch) and allowlisted origins are served.
    """
    if (
        request.headers.get("origin", "").strip()
        and not _check_origin(request)
        and not _is_allowed_extension_origin(request)
    ):
...
def _install_gate(request: Request, authorization: Optional[str]) -> None:
    ...
    import hmac
    # CSRF / drive-by defense, applied BEFORE the loopback pass: a POST with no custom
    # headers is CORS-simple, so the browser sends it without a preflight and the
    # loopback check alone would execute it for any site the owner has open. A browser
    # always stamps Origin on a cross-origin POST/PUT; a foreign one is refused here
    # even from loopback. Origin-less callers (CLI, curl, adk) are unaffected.
    origin = (request.headers.get("origin") or "").strip()
    if origin and not _origin_allowed(origin):
        raise HTTPException(403, "cross-origin browser request not permitted")
    if not origin and (request.headers.get("sec-fetch-site") or "").lower() == "cross-site":
        raise HTTPException(403, "cross-site browser request not permitted")
    ...
    if token and provided and hmac.compare_digest(provided, token):
        return
    raise HTTPException(403, "requires a loopback caller or the enrolled bearer token")
...
@app.post("/license/validate", dependencies=[Depends(_require_node_owner)])
...
def _openai_backend_for(model: str) -> Optional[str]:
    m = (model or "").lower()
    # THE OPERATOR'S OWN BACKEND IS RUNG 0. Checked before the model-name special case
    # as well: someone who configured a backend configured it for every turn, and a
    # browser that asks for "bonsai" by habit should not be the thing that silently
    # routes them somewhere else. `_resolve_backend_model` rewrites the model id to the
    # one they configured, so a name this backend does not serve does not 404.
    if _state.custom and _CUSTOM["base_url"]:
        return _CUSTOM["base_url"]
...
async def _resolve_backend_model(base: str, model: str) -> str:
    ...
    # The custom backend serves what its OWNER configured, and the caller's model id is
    # usually a Bonsai name the browser inherited from the in-browser catalogue. Rewrite
    # it here rather than letting the upstream 404 on a name it never heard of.
    if _CUSTOM["base_url"] and base.rstrip("/") == _CUSTOM["base_url"] and _CUSTOM["model"]:
        return _CUSTOM["model"]
...
@app.post("/v1/chat/completions", dependencies=[Depends(_require_node_owner)])
async def openai_chat_completions(request: Request):
    ...
    # Empty for every backend but the operator's own (see `_custom_auth_headers`).
    up_headers = _custom_auth_headers(base)
    if bool(payload.get("stream")):
        client = httpx.AsyncClient(timeout=httpx.Timeout(connect=5.0, read=300.0, write=60.0, pool=5.0))
        try:
            upstream = await client.send(
                client.build_request("POST", url, json=payload, headers=up_headers), stream=True
            )
    ...
    async with httpx.AsyncClient(timeout=300.0) as c:
        try:
            r = await c.post(url, json=payload, headers=up_headers)
        except httpx.HTTPError as e:
            raise HTTPException(502, f"inference backend error: {e}") from e
        return Response(
            content=r.content,
...
def _check_origin(request) -> bool:
    """Verify the request Origin is a pinned first-party https origin or loopback.
...
# Browser-extension origins. A web page cannot forge these: Chrome/Firefox set
# them only for requests made by an installed extension's own pages / service
# worker. The first-party awconnect extension (MV3 service worker) lists this
# node's tools via GET /mcp/tools with `Origin: chrome-extension://<id>`.
# AWNODE_EXTENSION_IDS (comma-separated) pins the accepted ids; unset, any
# extension id is accepted (awconnect ships unpacked/dev builds with varying ids).
_EXTENSION_SCHEMES = ("chrome-extension", "moz-extension", "safari-web-extension")


def _is_allowed_extension_origin(request) -> bool:
    """True when the Origin is a browser-extension origin (optionally pinned)."""
    origin = request.headers.get("origin", "").strip().lower()
    if not origin:
        return False
    parsed = urlparse(origin)
    if parsed.scheme not in _EXTENSION_SCHEMES:
        return False
    ext_id = parsed.netloc
    if not ext_id or not re.fullmatch(r"[a-z0-9-]{1,64}", ext_id):
        return False
    pinned = {
        x.strip().lower()
        for x in os.environ.get("AWNODE_EXTENSION_IDS", "").split(",")
        if x.strip()
    }
    return not pinned or ext_id in pinned
